...
当前应用是 Vite 纯前端调用链。浏览器边缘在用户点击“排盘”时一次读取完整当前时刻，直接构造一个 `BeijingDateTime` 并注入 `mingpan`；`mingpan` 从同一个值派生当前公历年月，为每步大运组装连续十个大运关联流年，为每个流年组装十二个流月，并分别标记当前大运、今年与当前流月。`paipan`、`mingpan`、`liunian` 与 `liuyue` 都不直接读取系统时钟。
