// 寿星太阳私有核心。
// 数据与算法机械移植自寿星天文历提交
// c55a3f62b2831887066340e0b80a5fd0ef6f15e1（原作者：许剑伟）。
// 仅保留太阳坐标、太阳视黄经反求、章动、光行差、ΔT、儒略日与高精度时差。

/**
 * 儒略日工具：仅保留天文 facade 所需的 J2000 常量与格里历日期 -> 儒略日转换。
 * 上游其余反推日期、星期等能力不属于太阳最小依赖闭包。
 */
class JulianDay {
...
class SolarCore {
    ...
    static nutationLon2(t: number): number {
        ...
        const t2: number = t * t;
        let dl: number = 0;
        for (let i: number = 0, j: number = SolarCore.NUT_B.length; i < j; i += 5) {
            dl += (SolarCore.NUT_B[i + 3]! + a) * Math.sin(SolarCore.NUT_B[i]! + SolarCore.NUT_B[i + 1]! * t + SolarCore.NUT_B[i + 2]! * t2);
            a = 0;
        }
        return dl / 100 / SolarCore.SECOND_PER_RAD;
    }
    ...
    static eLon(t: number, n: number): number {
        ...
        let c: number;
        let n0: number;
        const m0: number = SolarCore.XL0[2]! - SolarCore.XL0[1]!;
        for (let i: number = 0; i < 6; i++, tn *= t) {
            n1 = ~~(SolarCore.XL0[1 + i]!);
            n2 = ~~(SolarCore.XL0[2 + i]!);
            n0 = n2 - n1;
            if (n0 === 0) {
                continue;
            }
            ...
            for (let j: number = n1; j < m; j += 3) {
                c += SolarCore.XL0[j]! * Math.cos(SolarCore.XL0[j + 1]! + t * SolarCore.XL0[j + 2]!);
            }
            v += c * tn;
        }
        v /= SolarCore.XL0[0]!;
        const t2: number = t * t;
        return v + (-0.0728 - 2.7702 * t - 1.1019 * t2 - 0.0996 * t2 * t) / SolarCore.SECOND_PER_RAD;
    }

    static gxcSunLon(t: number): number {
        const t2: number = t * t;
        return -20.49552 * (1 + (0.016708634 - 0.000042037 * t - 0.0000001267 * t2) * Math.cos(-0.043126 + 628.301955 * t - 0.000002732 * t2)) / SolarCore.SECOND_PER_RAD;
    }
    ...
    static saLon(t: number, n: number): number {
        return SolarCore.eLon(t, n) + SolarCore.nutationLon2(t) + SolarCore.gxcSunLon(t) + Math.PI;
    }
    ...
    static dtCalc(y: number): number {
        const size: number = SolarCore.DT_AT.length;
        const y0: number = SolarCore.DT_AT[size - 2]!;
        const t0: number = SolarCore.DT_AT[size - 1]!;
        if (y >= y0) {
            const jsd: number = 31;
            if (y > y0 + 100) {
                return SolarCore.dtExt(y, jsd);
            }
            return SolarCore.dtExt(y, jsd) - (SolarCore.dtExt(y0, jsd) - t0) * (y0 + 100 - y) / 100;
        }
        let i: number;
        for (i = 0; i < size; i += 5) {
            if (y < SolarCore.DT_AT[i + 5]!) {
                break;
            }
        }
        const t1: number = (y - SolarCore.DT_AT[i]!) / (SolarCore.DT_AT[i + 5]! - SolarCore.DT_AT[i]!) * 10;
        const t2: number = t1 * t1;
        const t3: number = t2 * t1;
        return SolarCore.DT_AT[i + 1]! + SolarCore.DT_AT[i + 2]! * t1 + SolarCore.DT_AT[i + 3]! * t2 + SolarCore.DT_AT[i + 4]! * t3;
    }

    static dtT(t: number): number {
        return SolarCore.dtCalc(t / 365.2425 + 2000) / SolarCore.SECOND_PER_DAY;
    }
    ...
    static saLonT(w: number): number {
        ...
        v = SolarCore.ev(t);
        t += (w - SolarCore.saLon(t, 10)) / v;
        return t + (w - SolarCore.saLon(t, -1)) / SolarCore.ev(t);
    }

    static qiAccurate(w: number): number {
        const t: number = SolarCore.saLonT(w) * 36525;
        return t - SolarCore.dtT(t) + SolarCore.ONE_THIRD;
    }

    static normalizedRadians(value: number): number {
        const normalized = value % SolarCore.PI_2;
        return normalized < 0 ? normalized + SolarCore.PI_2 : normalized;
    }

    static signedRadians(value: number): number {
        const normalized = value % SolarCore.PI_2;
        if (normalized <= -Math.PI) return normalized + SolarCore.PI_2;
        if (normalized > Math.PI) return normalized - SolarCore.PI_2;
        return normalized;
    }

    static trueObliquity(t: number): number {
        const t2 = t * t;
        const t3 = t2 * t;
        const t4 = t3 * t;
        const t5 = t4 * t;
        return (
            ...
            - 5.76e-7 * t4
            - 4.34e-8 * t5
        ) / SolarCore.SECOND_PER_RAD;
    }

    static rotateEclipticToEquatorial(
        longitude: number,
        latitude: number,
        obliquity: number,
    ): [number, number] {
        ...
        return [SolarCore.normalizedRadians(rightAscension), declination];
    }
    ...
    static equationOfTime(t: number): number {
        ...
        let meanLongitude = (
            ...
            - 0.1124 * t4
            - 0.00009 * t5
        ) / 1_000_000_000 + Math.PI - 20.5 / SolarCore.SECOND_PER_RAD;
        const longitudeNutation =
            -17.2 * Math.sin(2.1824 - 33.75705 * t) / SolarCore.SECOND_PER_RAD;
        const obliquityNutation =
            9.2 * Math.cos(2.1824 - 33.75705 * t) / SolarCore.SECOND_PER_RAD;
        const obliquity = SolarCore.trueObliquity(t) + obliquityNutation;
        const solarLongitude =
            SolarCore.eLon(t, 50)
            + Math.PI
            + SolarCore.gxcSunLon(t)
            + longitudeNutation;
        const solarLatitude = -(
        ...
        const [rightAscension] = SolarCore.rotateEclipticToEquatorial(
            solarLongitude,
            solarLatitude,
            obliquity,
        );
        const apparentRightAscension =
            rightAscension - longitudeNutation * Math.cos(obliquity);
        meanLongitude = SolarCore.signedRadians(meanLongitude - apparentRightAscension);
        return meanLongitude / SolarCore.PI_2;
    }
}

export function julianDayFromFields(
    year: number,
    month: number,
    day: number,
    hour: number,
    ...
    return JulianDay.fromYmdHms(year, month, day, hour, minute, second).getDay();
}

/** 返回以 J2000 为零点、按北京时间日期分解的交节日数。 */
export function solarLongitudeMomentBeijingDays(targetLongitude: number): number {
    return SolarCore.qiAccurate(targetLongitude);
}

/** 传入 UT 的 J2000 日数，返回真太阳时减平太阳时的完整浮点日数。 */
export function equationOfTimeDays(universalJ2000Days: number): number {
    const dynamicalDays = universalJ2000Days + SolarCore.dtT(universalJ2000Days);
    return SolarCore.equationOfTime(dynamicalDays / 36525);
}
