 ...
 * 返回分钟数（可为负、为分数）。
 */
export function 真太阳时偏移分钟数(birthClockUtcMs: number, longitude: number): number {
  return 经度修正分钟数(longitude) + 均时差分钟数(birthClockUtcMs);
}
 ...
 * 来对比干支切换点，故只做平移、不改时区）。
 */
export function 应用真太阳时(birthClockUtcMs: number, longitude: number): number {
  return birthClockUtcMs + 真太阳时偏移分钟数(birthClockUtcMs, longitude) * 60_000;
}
