...
import { 经度修正分钟数, 应用经度修正, 参考经度, 每度分钟数, 真太阳时偏移分钟数, 应用真太阳时 } from "../src/solar-time.js";
...
describe("真太阳时合成（经度修正 + 均时差）(#15)", () => {
  ...
  it("中央经线 120° + 均时差零点日期（4-15）-> 总偏移 ≈ 0、时间戳不变", () => {
    ...
    const offset = 真太阳时偏移分钟数(t, 120);
    expect(Math.abs(offset)).toBeLessThan(2); // 均时差零点附近 ±2 分
    expect(应用真太阳时(t, 120)).toBe(t + offset * 60_000);
  });
  ...
  it("喀什 76° + 11 月初均时差极值 -> 总偏移 ≈ 经度修正 + 均时差 ≈ −160 分", () => {
    ...
    const lng = 75.996;
    const offset = 真太阳时偏移分钟数(t, lng);
    const expected = 经度修正分钟数(lng) + 均时差分钟数(t);
  ...
  it("同一 UTC 时刻不同经度 -> 总偏移之差 = 经度修正之差（均时差与经度无关）", () => {
    ...
    const o1 = 真太阳时偏移分钟数(t, 75.996);
    const o2 = 真太阳时偏移分钟数(t, 131.165);
    const eot = 均时差分钟数(t);
    // o1 = 经度修正(76°) + eot, o2 = 经度修正(131°) + eot -> o2 - o1 = 经度修正差
    expect(o2 - o1).toBeCloseTo(经度修正分钟数(131.165) - 经度修正分钟数(75.996), 4);
    // 均时差分量相同：o1 - 经度修正(76°) = o2 - 经度修正(131°) = eot
    expect(o1 - 经度修正分钟数(75.996)).toBeCloseTo(eot, 4);
    expect(o2 - 经度修正分钟数(131.165)).toBeCloseTo(eot, 4);
  });
  ...
  it("应用真太阳时 = 钟表时时间戳 + 总偏移分钟数 × 60_000 ms", () => {
    ...
    const offset = 真太阳时偏移分钟数(t, lng);
    expect(应用真太阳时(t, lng)).toBe(t + offset * 60_000);
  });
  ...
  it("真太阳时 − 平太阳时 = 均时差（验证两层关系）", () => {
    ...
    const 平太阳 = 应用经度修正(t, lng);
    const 真太阳 = 应用真太阳时(t, lng);
    expect(真太阳 - 平太阳).toBeCloseTo(均时差分钟数(t) * 60_000, 0);
  });
});
