...
describe("CLI - 流年 (T9)", () => {
  ...
  // T12 起每柱干支后括注十神（天干十神 + 藏干十神），断言每行 `\d{4} .{2}（…）` 格式。
  it("默认打印流年块：标题行 + 10 行 '<year> <干支>（十神括注）'", () => {
    const { stdout, exit } = runCli(["-y", "2000", "-m", "1", "-d", "1", "-H", "12", "-M", "0"]);
    expect(exit).toBe(0);
    // 标题行：流年（今年 <year> 起，10 柱）：
    expect(stdout).toMatch(/^流年（今年 \d+ 起，10 柱）：$/m);
    // 10 行：两空格缩进 + 年份 + 空格 + 两字干支 + 十神括注（天干·十神；支·藏干十神 …）
    const 天干类 = "[甲乙丙丁戊己庚辛壬癸]";
    const 地支类 = "[子丑寅卯辰巳午未申酉戌亥]";
    const 流年行 = stdout
      .split("\n")
      .filter((l) =>
        new RegExp(`^\\s+\\d+ ${天干类}${地支类}（.·[^；]+；.·.+）$`).test(l),
      );
