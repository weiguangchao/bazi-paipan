...
import { 十神, 藏干表 } from "../src/shishen.js";
...
describe("十神 - 规则穷举 10 种十神（日主=戊，阳土）", () => {
  ...
  it("比肩：戊（同五行、同阴阳）-> 比肩", () => {
    expect(十神(日主, "戊子").天干十神).toBe("比肩");
  });

  it("劫财：己（同五行、异阴阳）-> 劫财", () => {
    expect(十神(日主, "己丑").天干十神).toBe("劫财");
  });

  it("食神：庚（我生金、同阴阳）-> 食神", () => {
    expect(十神(日主, "庚辰").天干十神).toBe("食神");
  });

  it("伤官：辛（我生金、异阴阳）-> 伤官", () => {
    expect(十神(日主, "辛未").天干十神).toBe("伤官");
  });

  it("偏财：壬（我克水、同阴阳）-> 偏财", () => {
    expect(十神(日主, "壬戌").天干十神).toBe("偏财");
  });

  it("正财：癸（我克水、异阴阳）-> 正财", () => {
    expect(十神(日主, "癸亥").天干十神).toBe("正财");
  });

  it("七杀：甲（克我木、同阴阳）-> 七杀", () => {
    expect(十神(日主, "甲寅").天干十神).toBe("七杀");
  });

  it("正官：乙（克我木、异阴阳）-> 正官", () => {
    expect(十神(日主, "乙卯").天干十神).toBe("正官");
  });

  it("偏印：丙（生我火、同阴阳）-> 偏印", () => {
    expect(十神(日主, "丙巳").天干十神).toBe("偏印");
  });

  it("正印：丁（生我火、异阴阳）-> 正印", () => {
    expect(十神(日主, "丁午").天干十神).toBe("正印");
  });
});
...
describe("十神 - 藏干十神（日主=戊，阳土）", () => {
  ...
  it("辰藏 戊乙癸 -> 比肩、正官、正财", () => {
    const r = 十神(日主, "庚辰");
    expect(r.藏干十神).toEqual(["比肩", "正官", "正财"]);
  });

  it("午藏 丁己 -> 正印、劫财", () => {
    const r = 十神(日主, "戊午");
    expect(r.藏干十神).toEqual(["正印", "劫财"]);
  });

  it("卯藏 乙 -> 正官（单藏干）", () => {
    const r = 十神(日主, "己卯");
    expect(r.藏干十神).toEqual(["正官"]);
  });
});
...
describe("藏干表 - 12 地支全量覆盖", () => {
  ...
  for (const [zhi, expectedGans] of Object.entries(expected)) {
    it(`${zhi} -> ${expectedGans.join("")}（${expectedGans.length} 个藏干）`, () => {
      expect(藏干表[zhi]).toEqual(expectedGans);
    });
  }

  it("12 地支全量覆盖（藏干数在 1-3）", () => {
    expect(Object.keys(藏干表)).toHaveLength(12);
    for (const gans of Object.values(藏干表)) {
