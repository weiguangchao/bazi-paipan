...
describe("排盘 - 大运顺行 10 柱", () => {
  ...
  it("阳年男 2000-03-10 12:00 -> 顺行 10 柱庚辰…己丑，起运规则不变", async () => {
    const result = await paipan({ year: 2000, month: 3, day: 10, hour: 12, minute: 0, gender: "男" });
  ...
  it("顺行起运年月：2000-03 出生起运 8岁6月 -> 第 0 柱 2008-09", async () => {
    const result = await paipan({ year: 2000, month: 3, day: 10, hour: 12, minute: 0, gender: "男" });
...
describe("排盘 - 大运逆行 10 柱", () => {
  ...
  it("阳年女 2000-03-10 12:00 -> 逆行 10 柱戊寅…己巳，起运 1岁6月", async () => {
    const result = await paipan({ year: 2000, month: 3, day: 10, hour: 12, minute: 0, gender: "女" });
    const { direction, qiyun, zhu } = result.dayun!;
  ...
  it("阴年男 2001-03-10 12:00 -> 逆行（阴年男逆）", async () => {
    const result = await paipan({ year: 2001, month: 3, day: 10, hour: 12, minute: 0, gender: "男" });
  ...
  it("阴年女 2001-03-10 12:00 -> 顺行（阴年女顺）", async () => {
    const result = await paipan({ year: 2001, month: 3, day: 10, hour: 12, minute: 0, gender: "女" });
...
describe("排盘 - 起运岁命例 (T6)", () => {
  ...
  it("立春后出生阳男 -> 顺行数到惊蛰，起运 9岁11月", async () => {
    const result = await paipan({ year: 2000, month: 2, day: 5, hour: 4, minute: 41, gender: "男" });
    expect(result.dayun!.direction).toBe("顺");
  ...
  it("立春前出生（1999己卯阴年）男 -> 逆行（按归属年干己判定）", async () => {
    const result = await paipan({ year: 2000, month: 1, day: 1, hour: 12, minute: 0, gender: "男" });
...
describe("排盘 - 无性别不算大运 (T6)", () => {
  it("未给 gender -> 大运 undefined，四柱仍正常", async () => {
...
describe("大运 - 纯函数单测 (T6)", () => {
  // 直接调用 大运 纯函数，绕过排盘前置。
  it("月柱 戊寅、阳年（年干戊=4 阳年）男顺行 -> 第 0 柱 己卯", async () => {
    const r = await dayun("戊寅", 4, "男", Date.UTC(2000, 1, 5, 4, 41) - 8 * 3600 * 1000, 2000, 2);
    expect(r.direction).toBe("顺");
    expect(r.zhu[0]!.ganzhi).toBe("己卯");
  });

  it("大运方向穷举：偶数年干男顺、奇数年干男逆", async () => {
