...
describe("排盘 - 日柱 (T1)", () => {
  ...
  it("子正跨日：2000-01-01 23:59 → 戊午；2000-01-02 00:00 → 己未", () => {
    const 晚 = paipan({ year: 2000, month: 1, day: 1, hour: 23, minute: 59 });
    const 早 = paipan({ year: 2000, month: 1, day: 2, hour: 0, minute: 0 });
    expect(晚.rizhu).toBe("戊午"); // 23:59 仍属 1 日
    expect(早.rizhu).toBe("己未"); // 00:00 已属 2 日
  });
...
describe("排盘 - 年柱 (T2)", () => {
  ...
  it("跨立春边界：2000-02-05 03:00 -> 己卯；2000-02-05 06:00 -> 庚辰", () => {
    const 前 = paipan({ year: 2000, month: 2, day: 5, hour: 3, minute: 0 });
    const 后 = paipan({ year: 2000, month: 2, day: 5, hour: 6, minute: 0 });
    expect(前.nianzhu).toBe("己卯");
    expect(后.nianzhu).toBe("庚辰");
  });
});
...
describe("排盘 - 真太阳时合成（经度修正 + 均时差）(T5/#15)", () => {
  ...
  it("给/不给喀什地名 -> 时柱不同：钟表 12:00 戊午 vs 真太阳时 丁巳", () => {
    const 钟表 = paipan({ year: 2000, month: 1, day: 1, hour: 12, minute: 0 });
    const 真太阳 = paipan({
      year: 2000, month: 1, day: 1, hour: 12, minute: 0,
      birthplace: { province: "新疆维吾尔自治区", city: "喀什地区" },
    });
    expect(钟表.shizhu).toBe("戊午");
    expect(真太阳.shizhu).toBe("丁巳");
    expect(钟表.shizhu).not.toBe(真太阳.shizhu);
    // 经度修正不影响日柱（真太阳时仍属同一历法日）
    expect(真太阳.rizhu).toBe("戊午");
    expect(真太阳.longitudeCorrectionApplied).toBe(true);
  });
  ...
  it("双鸭山经度修正跨子正 -> 日柱/时柱/近子正同时变化", () => {
    const 钟表 = paipan({ year: 2000, month: 1, day: 1, hour: 23, minute: 20 });
    const 真太阳 = paipan({
      year: 2000, month: 1, day: 1, hour: 23, minute: 20,
      birthplace: { province: "黑龙江省", city: "双鸭山市" },
    });
    expect(钟表.rizhu).toBe("戊午");
    expect(钟表.shizhu).toBe("壬子");
    expect(钟表.nearZizheng).toBe(false);
    expect(钟表.longitudeCorrectionApplied).toBe(false);

    expect(真太阳.rizhu).toBe("己未");
    expect(真太阳.shizhu).toBe("甲子");
    expect(真太阳.nearZizheng).toBe(true);
    expect(真太阳.longitudeCorrectionApplied).toBe(true);
  });
  ...
  it("北京出生：经度修正 + 均时差幅度小，时柱不变但经度修正 true", () => {
    ...
    expect(真太阳.shizhu).toBe(钟表.shizhu);
    expect(真太阳.longitudeCorrectionApplied).toBe(true);
  });
  ...
  it("均时差使近子正真太阳时跨子正 -> 日柱/时柱同时切换（常州 11 月初）", () => {
    const 钟表 = paipan({ year: 2026, month: 11, day: 3, hour: 23, minute: 48 });
    const 真太阳 = paipan({
      year: 2026, month: 11, day: 3, hour: 23, minute: 48,
      birthplace: { province: "江苏省", city: "常州市" },
    });
    expect(钟表.rizhu).toBe("辛巳");
    expect(钟表.shizhu).toBe("戊子");
    expect(钟表.longitudeCorrectionApplied).toBe(false);
    // 均时差把真太阳时推过子正 -> 日柱、时柱同时切换
    expect(真太阳.rizhu).toBe("壬午");
    expect(真太阳.shizhu).toBe("庚子");
    expect(真太阳.longitudeCorrectionApplied).toBe(true);
    // 两侧均近子正（钟表 23:48 距子正 12 分、真太阳刚过子正）
    expect(钟表.nearZizheng).toBe(true);
    expect(真太阳.nearZizheng).toBe(true);
  });
