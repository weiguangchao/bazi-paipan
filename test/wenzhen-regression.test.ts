...
// 每市一条用例，断言我们的"应用trueSolarTime时"（经 #15 实现）在该市经度 + 取样时刻下
// 与问真trueSolarTime时吻合。经度修正与均时差合成的正确性由此获得外部锚点，回归可见、
// 某市失败可精确定位、不影响其他市。
//
// 词汇遵循 CONTEXT.md：trueSolarTime时 = clock时 + 经度修正 + 均时差。
//
// ── 容差设计（issue 验收 + 实测分析）──────────────────────────────────────
//
// 1. trueSolarTime时吻合（核心锚点）：用问真经度计算我们的 trueSolarTime时偏移分钟数，与问真
//    给出的整分偏移相比，容差 ≤ 2 分。该容差吸收三重误差来源：
//      a) 问真trueSolarTime时显示取整到分钟 -> ±0.5 分舍入；
...
//         非 bug，属 #14 已定范围）。
//    去除地理编码差异（用问真经度而非 city-geo 经度代入）after，337 市residual落在
...
//      b) ≥ 0.5°：真"少数市"（实测 18 市），须出现在下方"knownGeocodingDifferences清单"
...
// trueSolarTime时主要由经度修正决定，是干净的合成回归目标）。
...
// clock时 12:00 对应的基准分钟（距 0:00）。wenzhenOffset以此为基准比较。
const CLOCK_NOON_MIN = 12 * 60;

// 经度"地理编码微小差异"吸收阈值（度）。< 此值直接通过；≥ 此值须在known清单中。
const longitudeToleranceDegrees = 0.5;

// trueSolarTime时吻合容差（分钟）。
const trueSolarTimeToleranceMinutes = 2;

// known清单差值与实测一致性的允许抖动（度），防止清单陈旧误报。
const knownDifferenceJitterDegrees = 0.1;

// knownGeocodingDifferences清单：city-geo 与问真对同一市使用了不同地理编码
...
/** 在knownGeocodingDifferences清单中lookup指定省/市，返回其记录的longitudeDifference值（度）。 */
...
describe("问真对照回归测试集（数据驱动 338 市）(#18)", () => {
  // 数据文件完整性先行校验：避免因 JSON 缺失/损坏导致after续用例空跑。
  ...
  describe.each(provinces)("省：%s", (province: string) => {
    ...
    it.each(cases)("%s：经度吻合 + trueSolarTime时吻合", (_cityName: string, c: WenzhenCity) => {
      ...
      if (longitudeDifference >= longitudeToleranceDegrees) {
        // 超阈值：必须出现在knownGeocodingDifferences清单中，否则视为 city-geo 回归。
        const known = findKnownDifference(c.province, c.city);
        expect(
          known,
          `${c.province}/${c.city} longitudeDifference ${longitudeDifference.toFixed(4)}° 超阈值却不在known差异清单`,
        ).toBeDefined();
      ...
      // city-geo 为准：经度断言以 city-geo 为真值（上方）；下方trueSolarTime时断言改用
      // 问真经度代入，刻意去除 city-geo 与问真地理编码差异的干扰，使residual只剩
      // EoT 公式差（常数）+ 取整/经度小数舍入，便于用窄容差回归 EoT 合成。

      // ── trueSolarTime时吻合：用问真经度算actualOffset，与问真整分偏移比 ─────────
      // 去除地理编码差异after，residual只剩 EoT 公式差（常数）+ 取整/经度小数舍入，
      // 容差 ≤ 2 分吸收之。
  ...
  // 覆盖度守卫：防止 wenzhen-338.json 被静默删减条目after测试仍"全过"。
